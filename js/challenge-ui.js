...
function chDialogItemFilter(category) {
	...
	switch (category) {
		...
		case 12: types=[BULGEM,BULGEL,AAFD,LANDINGCRAFT,LANDINGTANK,WG42,SRF,FCF,DRUM,SCAMP,REPAIR,SUBRADAR,TRANSPORTITEM,RATION,OILDRUM,TRANSPORTITEM2,MINESWEEPER]; break;
	}
...
function chSetEquip(itemid) {
	...
	if (item && DIALOGFLEETSEL == 5) { //LBAS only
		...
		var num = (type == SEAPLANE || type == CARRIERSCOUT || type == FLYINGBOAT || type == LANDSCOUT) ? 4 : (type == LANDBOMBERL ? 9 : (MAPDATA[WORLD].lbasSlotCount || 18));
...
function chGetLBASNumPlanes(item) {
	...
	let num = (type == SEAPLANE || type == CARRIERSCOUT || type == FLYINGBOAT || type == LANDSCOUT)? 4 : (type == LANDBOMBERL ? 9 : (MAPDATA[WORLD].lbasSlotCount || 18));
	return num;
}
...
function chProcessKC3File2() {
	...
	if (MAPDATA[EVENTNUM].unlockDefault) CHDATA.event.unlocked = MAPDATA[EVENTNUM].unlockDefault;
		...
		for (let n=1; n<100; n++) {
			if (!MAPDATA[EVENTNUM].maps[n]) { CHDATA.event.unlocked = n; break; }
		}
	}
	...
	for (var sid in kcdata.ships) {
		...
		// TODO - apply the inverse of this operation if alt data format is imported (marriage hp isn't carried over, is bug)
	...
	if (MAPDATA[CHDATA.event.world].allowLBAS) {
		CHDATA.fleets[5] = [];
		for (var i=1; i<=(MAPDATA[CHDATA.event.world].lbasPlayableCap || 3); i++) {
...
function chStart() {
	...
	let supDataN = MAPDATA[WORLD].maps[MAPNUM].support2N, supDataB = MAPDATA[WORLD].maps[MAPNUM].support2B;
	if (supDataN && supDataN.comps[CHDATA.event.maps[MAPNUM].diff] && (supDataN.ldOnly ? CHDATA.event.maps[MAPNUM].hp <= getMapLDHP(WORLD,MAPNUM,CHDATA.event.maps[MAPNUM].diff,CHDATA.event.maps[MAPNUM].part) : true)){
		let enemies = [];
		for(let sid of MAPDATA[WORLD].maps[MAPNUM].support2N.comps[CHDATA.event.maps[MAPNUM].diff]){
	...
	if (supDataB && supDataB.comps[CHDATA.event.maps[MAPNUM].diff] && (supDataB.ldOnly ? CHDATA.event.maps[MAPNUM].hp <= getMapLDHP(WORLD,MAPNUM,CHDATA.event.maps[MAPNUM].diff,CHDATA.event.maps[MAPNUM].part) : true)){
		let enemies = [];
		for(let sid of MAPDATA[WORLD].maps[MAPNUM].support2B.comps[CHDATA.event.maps[MAPNUM].diff]){
			enemies.push(createDefaultShip(sid));
		}
	...
	if (MAPDATA[CHDATA.event.world].allowLBAS) {
		...
		for(let base of lbDat1){
			if(base.npc){
				++numBaseNF;
				SHIPDATA[5500+numBaseNF] = {
					name: 'Base NPC' + numBaseNF,
					image: (base.image ? base.image : 'LBAS1.png'),
					type: 'LandBase',
					SLOTS: (base.planeCount ? base.planeCount : [18,18,18,18]),
					fuel: 0,
					ammo: 0,
				};
				...
				if (numBaseActive < numBaseMax && CHDATA.fleets['lbas'+numBaseP] && LBAS1[LBAS1.length - 1].equips.length) {
					numBaseActive++;
					if (!LBAS1[LBAS1.length - 1]) continue;
					for (var j=0; j<LBAS1[LBAS1.length - 1].equips.length; j++) {
						var equip = LBAS1[LBAS1.length - 1].equips[j];
					...
					CHDATA.fleets['lbas'+numBaseP] = false;
					$('#btnLBAS'+numBaseP).css('opacity',.5);
				}
			}
		}
		let lbDat2 = (MAPDATA[CHDATA.event.world].maps[MAPNUM].lbasData2 && MAPDATA[CHDATA.event.world].maps[MAPNUM].lbasData2[CHDATA.event.maps[MAPNUM].diff]) ? MAPDATA[CHDATA.event.world].maps[MAPNUM].lbasData2[CHDATA.event.maps[MAPNUM].diff] : [  ];
		for(let base of lbDat2){
			++numBaseNE;
			var lb2 = new LandBase(base.planes,base.levels,base.profs,1,base.hp,base.ar,base.planeCount);
			LBAS2.push(lb2);
		}
...
function chLoadLBASPlayable(num) {
	...
	var lb = new LandBase(equips,improvs,profs);
	lb.PLANESLOTS = base.planes.slice();
	lb.planecount = base.planes.slice();
	lb.pos = num;
	return lb;
}

function chNewShipCount() {
	return {DD:0,CL:0,CLT:0,CA:0,CAV:0,BB:0,FBB:0,BBV:0,AV:0,SS:0,SSV:0,CVL:0,CV:0,CVB:0,LHA:0,AS:0,AR:0,AO:0,CT:0,DE:0,total:0,ids:[],aBB:0,aCV:0,CVE:0,speed:10};
}
...
function chTableSetShip(sid,fleet,slot,noswap) {
	...
	if (fleet == 5) { //LBAS only
		var lb = chLoadLBASPlayable(slot);
		$('#fleetlbac'+fleet+slot).text(lb.fleetAirPower(false,true));
		$('#fleetlbab'+fleet+slot).text(lb.airPowerDefend());
...
function chLoadSortieInfo(mapnum) {
	...
	if (barData.gauges && barData.gauges.hort){
		...
		$('#srtHPBarImg').attr('src', 'assets/bossbarTP.png');
	...
	if (CHDATA.config.diffmode == 1) {
		...
		switch(CHDATA.event.maps[mapnum].diff) {
			case 5:
				$('#srtDiffTitle').text(diffNames[5] || 'HISTORICAL');
	...
	if (CHDATA.config.disablelock || !(mapdata.giveLock||mapdata.checkLock||showHardLock)) {
		...
		if (mapdata.checkLock || showHardLock) {
			...
			for (var i=0; i<locks.length; i++) {
				$('#srtNoLock').append('<div style="display:inline-block"><img src="assets/maps/locks/lockno.png" style="position:absolute;height:40px"/><img src="assets/maps/locks/lock'+locks[i]+'.png" style="height:40px;vertical-align:middle"/></div>');
			}
		}
	}
...
function chSortieChangeDiff(diff) {
	...
	if ((diff == 3 || diff == 5) && WORLD != 36) {
		...
		delete CHDATA.event.maps[CHDATA.event.mapnum].debuffStatus;
